...
public class DistanceTimeFilterLocationProvider extends AbstractLocationProvider implements LocationListener {
    ...
    private static final String P_NAME = "com.rjp.cordova.bgloc";
    ...
    public DistanceTimeFilterLocationProvider(Context context) {
        super(context);
    ...
    public void onCreate() {
        ...
        //Force update PI
        forceUpdateGps = PendingIntent.getBroadcast(mContext, 0, new Intent(FORCE_GPS_ALARM_ACTION), PendingIntent.FLAG_CANCEL_CURRENT);
    ...
    public void onLocationChanged(Location location) {
        ...
        if(mConfig.getDistanceFilterTimeout() && lastKnownSpeed > mConfig.getDistanceFilterTimeoutMinSpeed() && !isRequestingSingle) {
            long maxDuration = round(scaledDistanceFilter / lastKnownSpeed * mConfig.getDistanceFilterTimeoutMultiplier()) * 1000;
            if(maxDuration < mConfig.getDistanceFilterTimeoutMin()) {
                maxDuration = mConfig.getDistanceFilterTimeoutMin();
            }

            if (mConfig.isDebugging()) {
                Toast.makeText(mContext, "Set timer: " + maxDuration + "ms", Toast.LENGTH_SHORT).show();
            }
        ...
        if(lastPost < System.currentTimeMillis() - mConfig.getFastestInterval()) {
    ...
    private Integer calculateDistanceFilter(Float speed) {
        ...
        if (speed < 100) {
            ...
            newDistanceFilter = (pow(roundedDistanceFilter, 2) / mConfig.getDistanceFilterDevide()) + (double) mConfig.getDistanceFilter();
        }
    ...
    private void requestSingleUpdate() {
        ...
        try {
            ...
            if (mConfig.isDebugging()) {
                Toast.makeText(mContext, "Request single Update", Toast.LENGTH_SHORT).show();
            }
    ...
    public void onDestroy() {
        logger.info("Destroying DistanceTimeFilterLocationProvider");
